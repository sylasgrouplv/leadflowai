...
(async () => {
  ...
  pass("A12 default automation rules present", ruleNames.join(",") === "appointment_confirmation,job_completed_review,lead_created_welcome,qualified_followup_sequence", ruleNames.join(","));
  ...
  pass("B3 re-run did not duplicate rules", (await repo.listAutomationRules(bizId)).length === 4, `n=${(await repo.listAutomationRules(bizId)).length}`);
  ...
  if (createdBiz) {
    ...
    pass("C9 default rules created by admin route", cRules.length === 4, `n=${cRules.length}`);
  }
